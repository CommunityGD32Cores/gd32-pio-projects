...
//#elif defined (__SAMD21E17A__) || defined(__SAMD21G18A__)  || defined(__SAMD21E18A__) || defined(__SAMD21J18A__)
  ...
  //generate from "Unique device ID (96 bits)".
  uint32_t pdwUniqueID[3]; //96 bits
  pdwUniqueID[0] = *(volatile uint32_t*)(0x1FFFF7E8); //32 bits
  pdwUniqueID[1] = *(volatile uint32_t*)(0x1FFFF7EC); //32 bits
  pdwUniqueID[2] = *(volatile uint32_t*)(0x1FFFF7EC); //32 bits
  // convert into 8-byte slices
  for (uint8_t i = 0; i < 3; i++)
  {
    id[i*4+0] = (uint8_t)(pdwUniqueID[i] >> 24);
    id[i*4+1] = (uint8_t)(pdwUniqueID[i] >> 16);
